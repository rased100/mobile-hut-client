...
const ManageAllOrders = () => {
    ...
    useEffect(() => {
        fetch("https://safe-mountain-74913.herokuapp.com/allOrders")
            .then((res) => res.json())
    ...
    const handleDelete = email => {
        ...
        if (proceed) {
            const url = `https://safe-mountain-74913.herokuapp.com/allOrders/${email}`
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
    ...
    const approveOrder = id => {
        const url = `https://safe-mountain-74913.herokuapp.com/myorders?id=${id}`
        fetch(url, {
            method: 'PUT',
